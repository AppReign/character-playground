...
describe("equipmentUploadPaths", () => {
  ...
  it("builds filename matching dotv path builder", () => {
    expect(
      buildEquipmentUploadFilename("plain-farmers-tunic", "male", "2h", "base")
    ).toBe("plain-farmers-tunic-2h-base-male.png");
    expect(
  ...
  it("builds object key matching dotv path builder", () => {
    expect(
      buildEquipmentUploadObjectKey(item, "male", "2h", "base")
    ).toBe(
      "equipment/plain-farmer/plain-farmers-tunic/male/plain-farmers-tunic-2h-base-male.png"
    );
  });
});
